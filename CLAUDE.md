...
- `preprocessed/` — regenerable caches organized by DOMAIN. `sources/<domain>/` holds the per-sequence serialized `.pt` (`real/`, `synt/{twin,gen}/`); `datasets/<domain>/` holds the assembled KiDKNet `data_dir`s (`real/`, `synt/{twin,gen}/`, `mixed/`). `gen/` holds synthetic sequences outside the paired real↔twin set, such as algorithmic forcegen. Sequence 04 is permanently excluded by the repository owner: neither real nor twin sequence-04 data may enter current or future experiments unless the owner explicitly reverses that decision. `mixed/` exists only under `datasets/` (there is no per-sequence mixed source). Batches are same-volume hardlinks: `datasets/real` + the real half of `datasets/mixed` link `sources/real`; `datasets/synt/twin` + the synt half of `datasets/mixed` link `sources/synt/twin`. Superseded sets are deleted outright (they regenerate via `assemble`).
...
- **Gap-reduction victory gate (owner ruling, 2026-08-10).** On the data-improvement line, an approach counts as a race winner ONLY once it demonstrates a verified data-side gap reduction on the task metric (gap-closed % under the frozen c2-baseline / c1-ceiling protocol). Until that evidence exists, its branches stay strictly local: no merge into master and no push to origin. Proxy metrics (feature separability drop, visual acceptance) do not qualify as victory; archival/preservation branches of retired routes stay local indefinitely unless the owner rules otherwise.
...
If `handoff/current.yaml` exists, read it first. Then read its `entrypoint`
and `state` targets in that order. `AGENTS.md` remains the authoritative
workspace ruleset. A handoff may narrow the active task and permissions, but
it may not override `AGENTS.md` or the user's current instructions.
