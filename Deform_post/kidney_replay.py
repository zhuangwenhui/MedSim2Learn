...
def render(ply_dir, camera_path, out_png_dir, size=None):
    ...
    """
    # The fixed-camera sequence renderer now lives on sim2vfp.Renderer so the
    # single proven recipe is shared; this stays the CLI entry point.
    from sim2vfp import Renderer
    n_ok = Renderer().render_fixed_camera_sequence(
        ply_dir, camera_path, out_png_dir, size=size
    )
