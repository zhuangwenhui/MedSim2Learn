...
class Renderer:
    ...
    def render_fixed_camera_sequence(
        ...
        Unlike the multiprocessing batch path (render_single_ply), this keeps one
        offscreen window for the whole sequence and re-applies the same fixed
        extrinsic every frame, so the camera stays a stationary laparoscope and
        only the surface deforms. The camera is loaded verbatim from
