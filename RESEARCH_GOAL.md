...
Also: **photometric augmentation = mild regularizer** (c1/c3 std -20~27%, mean neutral). **render-DR pilot designed (workflow w7lte239u) but BLOCKED** — no headless GL/EGL on this container; needs the Windows/GPU render container.
Two decisions await the user: (A) measurement-overhaul (enlarge val + force-norm + multi-fold/init k-shot, then re-baseline = ~full-day compute); (B) render-DR pilot (needs render env). Note: H100 yielded to a colleague mid-run (2026-06-22 ~11:37); our work confined to the 3 Ada since.
