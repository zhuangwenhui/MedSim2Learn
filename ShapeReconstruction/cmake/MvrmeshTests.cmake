...
mvrmesh_copy_gmsh_runtime(mvrmesh_smoke_tests)
...
if(MVRMESH_ENABLE_TETGEN)
    ...
    mvrmesh_copy_gmsh_runtime(mvrmesh_tetgen_evaluator_tests)
...
if(MVRMESH_ENABLE_GMSH)
    add_executable(mvrmesh_gmsh_evaluator_tests
        verification/backends/gmsh/gmsh_evaluator_tests.cpp
    )

    target_link_libraries(mvrmesh_gmsh_evaluator_tests PRIVATE mvrmesh)
    mvrmesh_copy_gmsh_runtime(mvrmesh_gmsh_evaluator_tests)

    add_test(NAME mvrmesh_gmsh_evaluator COMMAND mvrmesh_gmsh_evaluator_tests)
endif()
...
if(MVRMESH_ENABLE_CGAL)
    ...
    mvrmesh_copy_gmsh_runtime(mvrmesh_cgal_pmp_tests)
...
if(MVRMESH_ENABLE_TETGEN)
    add_test(
        NAME mvrmesh_cli_tetgen_metrics
        COMMAND ${CMAKE_COMMAND}
            -DCLI_EXE=$<TARGET_FILE:mvr_to_mesh_cli>
            -DINPUT_MVR=${MVRMESH_TEST_FIXTURE}
            -DOUTPUT_BASE=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_tetgen
            -DMETRICS_JSON=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_tetgen_metrics.json
            -P ${CMAKE_CURRENT_SOURCE_DIR}/verification/cmake/run_cli_tetgen_metrics.cmake
    )
endif()

if(MVRMESH_ENABLE_GMSH)
    add_test(
        NAME mvrmesh_cli_gmsh_metrics
        COMMAND ${CMAKE_COMMAND}
            -DCLI_EXE=$<TARGET_FILE:mvr_to_mesh_cli>
            -DINPUT_MVR=${MVRMESH_TEST_FIXTURE}
            -DOUTPUT_BASE=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_gmsh
            -DMETRICS_JSON=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_gmsh_metrics.json
            -P ${CMAKE_CURRENT_SOURCE_DIR}/verification/cmake/run_cli_gmsh_metrics.cmake
    )
endif()
...
add_test(
    NAME mvrmesh_cli_tetgen_switches_without_eval
    COMMAND ${CMAKE_COMMAND}
        -DCLI_EXE=$<TARGET_FILE:mvr_to_mesh_cli>
        -DINPUT_MVR=${MVRMESH_TEST_FIXTURE}
        -DOUTPUT_BASE=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_tetgen_switches_without_eval
        "-DEXPECTED_TEXT=--tetgen-switches requires --evaluate-tetgen"
        "-DCLI_EXTRA_ARGS=--tetgen-switches;pYQ"
        -DINCLUDE_EVALUATE_TETGEN=OFF
        -P ${CMAKE_CURRENT_SOURCE_DIR}/verification/cmake/expect_cli_failure_contains.cmake
)

add_test(
    NAME mvrmesh_cli_gmsh_algorithm_without_eval
    COMMAND ${CMAKE_COMMAND}
        -DCLI_EXE=$<TARGET_FILE:mvr_to_mesh_cli>
        -DINPUT_MVR=${MVRMESH_TEST_FIXTURE}
        -DOUTPUT_BASE=${CMAKE_CURRENT_BINARY_DIR}/tiny_cli_gmsh_algorithm_without_eval
        "-DEXPECTED_TEXT=--gmsh-algorithm3d requires --evaluate-gmsh"
        "-DCLI_EXTRA_ARGS=--gmsh-algorithm3d;10"
        -DINCLUDE_EVALUATE_TETGEN=OFF
        -P ${CMAKE_CURRENT_SOURCE_DIR}/verification/cmake/expect_cli_failure_contains.cmake
)
