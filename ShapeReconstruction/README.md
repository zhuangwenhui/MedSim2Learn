...
- **G3 (mesh control)**: increases or regularizes triangle count via configurable algorithms (`--adaptive-remesh`, `--cgal-mesh`)
...
│ tetrahedral │     │  • mvr_to_mesh_cli   │     │ (FEM solver) │     │ (ML)     │
...
Expected ctest output: `11/11 passed`.
...
| `--cgal-mesh` | off | flag | Enable CGAL Stage 1 (repair) + Stage 2 (isotropic remesh). Conflicts with `--adaptive-remesh`. |
...
`--cgal-mesh` and `--adaptive-remesh` are mutually exclusive (the CLI rejects them combined). All `--sharp-edge-degrees`, `--target-edge-length`, `--remesh-iterations` require `--cgal-mesh` to be present.
...
|---|---|---|---|
| `<input.ply>` (positional) | — | single | Input PLY for single-file evaluation. |
| `--matrix <ply1> <ply2> ...` | — | matrix | Variadic list of PLY inputs for matrix mode. |
...
| `--baseline P.ply` | — | matrix | Adds a labeled baseline row at the end of the comparison table. |
| `--label PATH=NAME` | — | matrix | Friendly label for a candidate PLY (default uses filename). Repeatable. |
...
The script runs 6 hardcoded candidates: `direct`, `adaptive_iter1`, `adaptive_iter3`, `cgal_default`, `cgal_L005` (target_edge_length 0.05), `cgal_L010` (target_edge_length 0.10). To change the candidate set, edit the `$candidates` array in the script.
...
Three meshing paths are exposed by `mvr_to_mesh_cli`. The following kidney.mvr V_surf numbers are from a real run (see `pressure_matrix.md` produced by `mvrmesh_pressure_matrix_kidney` ctest):
...
- kidney.mvr → V_surf 424 → 839 (iter1) → 5537 (iter3, 13× input)
- **When to use**: G3 increase is the priority and input mesh is already valid.

### 4.3 `--cgal-mesh`: CGAL 2-stage robust pipeline
...
```
                      ...
                      yes                        no
                       │                          │
                  --cgal-mesh                want triangle count up?
                       │                  ┌───────┴────────┐
              quality > count?           yes               no
              ┌────────┴────────┐         │                 │
             yes                no   --adaptive-remesh    direct
              │                 │
   --cgal-mesh         --cgal-mesh + small
   (default L)         --target-edge-length
```

For kidney.mvr specifically, `cgal_L005` (target 0.05) fails CGAL's `protect_constraints` precondition because the input mesh has sharp edges of length ~0.0676, exceeding the required 4/3 × 0.05 = 0.0667. See § 7 Troubleshooting.
...
2. **Evaluate** with `check_fem_pressure --matrix` → produces `pressure_matrix.md`
...
|---|---|---|---|
| direct | 424 | 24.7 MiB | 0.05× ✓ very light |
| adaptive_iter1 | 839 | 96.7 MiB | 0.18× ✓ light |
| adaptive_iter3 | 5537 | 4.20 GiB | 8.2× ✗ infeasible |
| cgal_default | 292 | 11.7 MiB | 0.02× ✓ extra light (but G3 reverses) |
| cgal_L010 | 140 | 2.7 MiB | 0.005× ✓ minimal (G3 strongly reversed) |
| **plate.ply (baseline)** | 1922 | 526 MiB | 1.00× |

`adaptive_iter1` is the closest non-baseline candidate to plate's complexity scale while keeping memory well under baseline — a reasonable G3 sweet spot.
...
|---|---|---|
| Memory peak (K + L) | `2 × (3·V_tet)² × 8 bytes` | Two dense matrices: K (stiffness) + L (= K⁻¹), held simultaneously by ADMM solver |
| DGETRI flops | `2 × (3·V_tet)³` | One-time LAPACK matrix inversion |
| DGEMV total flops | `2 × (3·V_tet)² × N_samples` | Per-sample force-to-displacement solve, summed over all samples |
...
```
DGETRI = DGEMV  ⟺  2·(3·V_tet)³ = 2·(3·V_tet)² × 22500
                ⟺  3·V_tet = 22500
...
- (3·V_tet)² = 1272² = 1,617,984
- Memory = 2 × 1,617,984 × 8 = 25,887,744 bytes ≈ 24.69 MiB
- DGETRI = 2 × 1272³ = 4,116,151,296 ≈ 4.12 × 10⁹
- DGEMV total = 2 × 1,617,984 × 22500 = 7.28 × 10¹⁰
...
`--sharp-edge-degrees` is too low for the input mesh. Common on small fixture meshes (e.g., the corner tetrahedron `tiny_surface.mvr` has all dihedral angles > 60°).
...
3. `mvr_to_mesh_cli.cpp` — add the new flag (in the `parse_args` switch + `CliOptions` struct + dispatch in `main`)
4. `verification/core/smoke_tests.cpp` — add a unit test in the anonymous namespace + call it from `main()`
5. `scripts/run_pressure_matrix.ps1` — add a row to the `$candidates` array (use a basename with no dots — see § 7.6)
6. `README.md` — update § 4 Algorithm Reference with the new algorithm's semantics + kidney V_surf result
...
4. `README.md` — update § 6 FEM Pressure Model with the formula + worked example
...
- `ctest --preset vs2022-x64-debug` passes 11/11
